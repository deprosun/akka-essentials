...
class InterceptingLogsSpec
  ...
  val creditCard = "1234-1234-1345-134"
  ...
  "A checkout flow" should {
    ...
    "freakout if the payment is denied" in {
...
object InterceptingLogsSpec {
  ...
  //the scenario is that we need to create an online checkout mechanism
  //upon request of checking out a merchandise, checkout actors:
  //  1. sends the request to payment manger to check validity
  //  2. upon validity from payment manager, checkout actor sends request to fulfillmentManager to full fill the order
  ...
  //testing this workflow is tricky.
  //we cant really use testprobes because checkoutActor creates from childactor
  //also checkout actor does not send any message back to the sender.. did checkout actor work?
  class CheckoutActor extends Actor {
    ...
    def awaitingCheckout: Receive = {
      case Checkout(item: String, creditCard: String) =>
  ...
  class FulfillmentManager extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      case DispatchOrder(item) =>
        orderId += 1
        // we need to test that this log line is being logged
